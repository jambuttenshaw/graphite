...
namespace Graphite
{
	...
	ShaderBytecode ShaderBytecode::NullShaderBytecode;

	ShaderBytecode::ShaderBytecode()
	{
		m_Bytecode = { nullptr, 0 };
	}

	ShaderBytecode::ShaderBytecode(IDxcBlob* shaderBlob)
	{
		m_Blob.Attach(shaderBlob);
		m_Bytecode = { m_Blob->GetBufferPointer(), m_Blob->GetBufferSize() };
	}
	...
	ShaderBytecode ShaderCompiler::CompileFromFile(const wchar_t* file, const wchar_t* entryPoint, ShaderType target) const
	{
		if (!file || !entryPoint)
		{
		...
		if (FAILED(result))
		{
			...
			return ShaderBytecode::NullShaderBytecode;
		}
		...
		return ShaderBytecode{ blob.Detach() };
	}
